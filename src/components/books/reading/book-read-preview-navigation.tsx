...
export default function BookReadPreviewNavigation({
  bookToc,
  chapterNumber,
  pageNumber,
}: Props) {
  ...
  const { prefetchBookReadPreview } = usePrefetch();

  useEffect(() => {
    if (!isLastPage) {
      prefetchBookReadPreview(bookId, next.chapter, next.page);
    }
  }, [bookId, isLastPage, next.chapter, next.page, prefetchBookReadPreview]);
