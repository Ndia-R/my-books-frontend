...
export default function Page() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!(username && email && password)) {
      if (username === '') setUsernameErrorMessage('ユーザー名は必須です。');
  ...
  const handleCheckUsername = async () => {
    ...
    if (await checkUsernameExists(usernameRef.current.value)) {
      setUsernameErrorMessage('そのユーザー名はすでに使われています。');
    }
  };
          ...
          <form className="flex w-full flex-col gap-y-4" onSubmit={handleSubmit}>
            <div>
              ...
              <Input
                ...
                className={cn(
                  'my-2 rounded-full',
                  usernameErrorMessage && 'border-destructive'
                )}
                id="username"
                name="username"
                autoComplete="off"
              ...
              {usernameErrorMessage && (
                <p className="text-xs text-destructive">{usernameErrorMessage}</p>
              )}
            </div>
